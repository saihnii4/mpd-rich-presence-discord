...
static inline uint64_t FNVHash(const char *str,
...
PresencePayload makePresence(const AppState &app, const TrackInfo *track,
                             bool isPaused, config cfg) {
...
std::string getParam(const std::vector<std::string> &args,
...
int main(int argc, char **args) {
  config_t cfg;
  configure("/home/pur0/.config/mpd-discord-richpresence/config.json", &cfg);

  auto vecArgs = std::vector<std::string>(args + 1, args + argc);

  if (isFlagSet(vecArgs, "--help") || isFlagSet(vecArgs, "-help") ||
      isFlagSet(vecArgs, "help")) {
#define LINE(what) std::cout << what << std::endl
    ...
    LINE("");
    LINE("Arguments:");
    LINE("  -h=ADDDRESS                         the address where the MPD "
         "server (defaults to 127.0.0.1)");
    LINE("  -p=PORT                             the port on which the target "
         "MPD server is listening (defaults to 6600)");
    LINE("  -P=PASSWORD                         the password to be sent after "
         "connection to the MPD server has been established in hopes of "
         "acquiring higher permissions. (default is empty, therfore no "
         "password sent.)");
    LINE("  --fork                              forks the process into the "
         "background.");
    LINE("  --no-idle                           Disables broadcasting of the "
         "idle state.");
    LINE("  --dont-broadcast-the-paused-state   Disables broadcasting of the "
         "paused state.");
...
#undef LINE
  ...
  state.BroadcastPaused =
      !isFlagSet(vecArgs, "--dont-broadcast-the-paused-state");
  ...
  while (true) {
    try {
      ...
      while (true) {
        updatePresence(mpd, &trackBuffer, state, cfg);
      ...
      std::cout << "Exception: " << e.what()
                << ". reconnecting to MPD in 5 seconds." << std::endl;
