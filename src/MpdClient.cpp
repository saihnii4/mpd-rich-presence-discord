...
void MpdClient::connect(const std::string &password) {
  ...
  if (!conn_)
    throw std::runtime_error("mpd_connection_get_error yielded success but "
                             "mpd_connection_new yielded a null pointer.");

  if (!password.empty()) {
    bool success = mpd_run_password(conn_, password.c_str());
    if (!success)
      throw std::runtime_error(
          "mpd_run_password failed. Is your password valid?");
  }
}
...
void MpdClient::waitForStateChange(const std::string &password) {
  while (true) {
    ...
    // we'll only reach this if mpd_run_idle_mask returns something other than
    // 0, meaning we'll get here when the player state changes
    return;
  }
}
...
MpdClient::State MpdClient::getState() {
  ...
  auto *mpdStatus = assertNotNull<mpd_status_t *>(
      mpd_recv_status, "mpd_recv_status in getState returned nullptr", conn_);
...
TrackInfo MpdClient::getCurrentTrack() {
  assertSuccess(mpd_command_list_begin,
                "mpd_command_list_begin in getCurrentTrack failed.", conn_,
                true);
  assertSuccess(mpd_send_status, "mpd_send_status in getCurrentTrack failed.",
                conn_);
  assertSuccess(mpd_send_current_song,
                "mpd_send_current_song in getCurrentTrack failed.", conn_);
  assertSuccess(mpd_command_list_end,
                "mpd_command_list_end in getCurrentTrack failed.", conn_);

  auto *mpdStatus = assertNotNull<mpd_status_t *>(
      mpd_recv_status, "mpd_recv_status in getCurrentTrackr returned nullptr",
      conn_);
  mpd_response_next(conn_);
  auto *mpdCurrentSong = assertNotNull<mpd_song_t *>(
      mpd_recv_song, "mpd_recv_status in getCurrentTrackr returned nullptr",
      conn_);
